...
m_ej_dyn_tot_sample = [5.e-4,1.e-3,5.e-3,0.01,0.05,0.1]  # PARAMETER [solar masses]
m_disk_sample = [3.e-3,3.e-2,5.e-2,0.1,0.3]   # PARAMETER
xi_w_sample = [0.01,0.05,0.1,0.2]    # PARAMETER 
xi_vis_sample = [0.10,0.20,0.30,0.40]    # PARAMETER 
...
kappa_dyn_max = 10.  #10.   # PARAMETER
s_dist_pc = 1.0e+1  # PARAMETER [pc]
ang_deg_sample=[15.0,30.0,45.0,60.0,75.0,90.0]     # PARAMETER 
...
#
#filter_name[12]  ='Y'
#filter_center[1]=1020   #[nm]
#
#filter_name[9]  ='L'
#filter_center[9]=3450   #[nm]
#
#filter_name[10]  ='M'
#filter_center[10]=4750   #[nm]
#
#filter_name[11]  ='N'
#filter_center[11]=10500   #[nm]
#
#filter_name[12]  ='Q'
#filter_center[12]=21000   #[nm]
...
def m_filter(name,T_ray,rad_ray,dist):
  for i in range(n_filters):
    if (name == filter_name[i]):
      break
    if (i == n_filters):
      sys.exit("Problem with the filter name!")
  fnu = calc_fnu(filter_center_nu[i],T_ray,rad_ray,dist)
  return -2.5*np.log10(fnu)-48.6

def calc_fnu(nu,T,rad,dist):
  tmp = 0.
  for k in range (n_k):
    tmp1 = flux_fact[k]*planckian(nu,T[k]) # [erg/cm^2/s/Hz]
...
for mm in range(len(ang_deg_sample)):


  ang_deg=ang_deg_sample[mm]

  # load the projection factor
  flux_file='./output_12_rays/flux_obs_ang_'+str(ang_deg)+'_nrays_'+str(n_k)+'.dat'

  tmp_col1,tmp_col2=np.loadtxt(flux_file,unpack=True,usecols=(0,4))

  flux_fact=np.zeros(n_k)
  for k in range(n_k):
    flux_fact[k]=float(tmp_col2[k])
    ...
    for jj in range(len(m_disk_sample)):
      ...
      for kk in range(len(xi_w_sample)):
        ...
        for ll in range(len(xi_vis_sample)):
          ...
          summary_file='summary/sum_'+str(ang_deg)+'_'+str(m_ej_dyn_tot)+'_'+str(m_disk)+'_'+str(xi_w)+'_'+str(xi_vis)+'.txt'
          output_file='output/out_'+str(ang_deg)+'_'+str(m_ej_dyn_tot)+'_'+str(m_disk)+'_'+str(xi_w)+'_'+str(xi_vis)+'.txt'

          lplot_name='lum_'+str(ang_deg)+'_'+str(m_ej_dyn_tot)+'_'+str(m_disk)+'_'+str(xi_w)+'_'+str(xi_vis)+'.png'
          mplot_name='mag_'+str(ang_deg)+'_'+str(m_ej_dyn_tot)+'_'+str(m_disk)+'_'+str(xi_w)+'_'+str(xi_vis)+'.png'
          # string with all the parameters
          parameter_string=[m_ej_dyn_tot,v_dyn_dist,kappa_dyn_min,kappa_dyn_max,m_disk,xi_w,v_wind_max,ye_dist_w,kappa_min_w,kappa_max_w,xi_vis,v_vis_max,kappa_min_vis,eps0,s_dist_pc,ang_deg]        
          
          # dynamic ejecta
          istat = mass_ang_dist(n_k,m_ej_dyn_tot,'sin2',m_ej_dyn)
          istat = vel_dist(n_k,'lin','unif_min',1.e-5,1.e-5,v_min_dyn)
          istat = vel_dist(n_k,'lin',v_dyn_dist,0.2,0.3,v_exp_dyn)
          ...
          istat = vel_dist(n_k,'lin','unif_min',1.e-5,1.e-5,v_min_w)
          ...
          istat = mass_ang_dist(n_k,m_ej_vis_tot,'cos2',m_ej_vis)
          istat = vel_dist(n_k,'lin','unif_min',1.e-5,1.e-5,v_min_vis)
          ...
          time_min=max(time_r_min)
          time_max=min(time_r_max)
          ...
          for i in range(n_time):
            for k in range(n_k):
              L_obs[i] = L_obs[i] + 4.*np.pi/solid_angle[k] * flux_fact[k]/np.pi * L_bol_tot[i,k]
          
          for i in range(n_time):
            for j in range(n_filters):
              mag_tot[i,j] = m_filter(filter_name[j],T_eff_tot[i,:],r_ph_tot[i,:],float(s_dist_cm))
          ...
          for i in range(n_time):
            ...
            f.write('%16e %16e %16e %16e %16e %16e %16e %16e %16e %16e %16e %16e %16e %16e\n' %(time[i],L_obs[i],lum_phys,mag_tot[i,0],mag_tot[i,1],mag_tot[i,2],mag_tot[i,3],mag_tot[i,4],mag_tot[i,5],mag_tot[i,6],mag_tot[i,7],mag_tot[i,8],mag_tot[i,9],mag_tot[i,10]))
         
          f.close
          
          g = open(summary_file,'w')
          g.write('%s %s %s \n' %(parameter_string,time_min,time_max))
          for i in range(n_filters):
            ntmp = np.argmin(mag_tot[:,i])
            g.write('%s %16e %16e\n' %(filter_name[i],time[ntmp]*sec2day,mag_tot[ntmp,i]))
...
#          fig=plt.figure(1)
#          for i in range(n_time):
#            Lum[i] = sum(L_bol_tot[i,:])
#          plt.loglog(time*sec2day,Lum,label='L_bol tot')
#          plt.loglog(time*sec2day,L_obs,label='L_obs iso')
#          plt.xlabel('time [d]')
#          plt.axis([0.1,50,1.e38,1.e43])
#          legend = plt.legend(loc='upper right', shadow=True)
#          plt.savefig('plots/'+lplot_name)         
 
#          fig=plt.figure(2)
#          for i in range(n_filters):
#            plt.semilogx(time*sec2day,mag_tot[:,i],label=filter_name[i])
#          plt.axis([0.1,50,25,15])
#          legend = plt.legend(loc='upper right', shadow=True)
#          plt.savefig('plots/'+mplot_name)      
